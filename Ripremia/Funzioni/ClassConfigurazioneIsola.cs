...
namespace EcoServiceApp {
    class ClassConfigurazioneIsola {
        public string TextLivelloMin;
        public string TextLivelloMax;
        public string TextMoltipl;
        public Boolean PortelloneFrontale;
        public string TextIndirizzoPortellone;
        public DataRow RowIsola;

        public class Bidone {
            public string Tipo="";
            public string SottoTipo = "";
            public string NumeroCentralina = "";
            public string PreMolt;
            public string TipoVisualizzazione;
            public string TipoVisualizzazioneConferimenti;
            public string Count;
            public string OrdineVisualizzazione;
        }
        public List<Bidone> ListaBidoni = new List<Bidone>();



        public Boolean LeggiConfigurazioneIsola(String IniStringa) {
            String IniFileConfigurazione = IniStringa;
            System.IO.MemoryStream IniMemSt = new System.IO.MemoryStream(System.Text.UTF8Encoding.UTF8.GetBytes(IniFileConfigurazione));
            IniFile Ini = new IniFile();
            Ini.Load(IniMemSt);
            TextLivelloMin = Ini.GetKeyValue("Generale", "LivelloMin");
            TextLivelloMax = Ini.GetKeyValue("Generale", "LivelloMax");
            TextMoltipl = Ini.GetKeyValue("Generale", "LivelloMoltiplicatore");
            var PresenzaPortellone = Ini.GetKeyValue("Generale", "Portellone");
            if (PresenzaPortellone == "")
                PortelloneFrontale = false;
            if (PresenzaPortellone == "true")
                PortelloneFrontale = true;
            if (PresenzaPortellone == "false")
                PortelloneFrontale = false;
            TextIndirizzoPortellone = Ini.GetKeyValue("Generale", "IndirizzoPortellone");
            ListaBidoni.Clear();
            for (var x = 0; x <= 20; x++) {
                var SecBidone = Ini.GetSection("Bidone" + x);
                if (SecBidone != null) {
                    Bidone BidoneTmp = new Bidone();
                    ListaBidoni.Add(BidoneTmp);
                    BidoneTmp.Tipo = SecBidone.GetKeyValue("Tipo");
                    BidoneTmp.SottoTipo = SecBidone.GetKeyValue("SottoTipo");
                    BidoneTmp.NumeroCentralina = SecBidone.GetKeyValue("Centralina");
                    BidoneTmp.PreMolt = SecBidone.GetKeyValue("LivelloPreMolt");
                    BidoneTmp.TipoVisualizzazione = SecBidone.GetKeyValue("TipoVisualizzazione");
                    BidoneTmp.TipoVisualizzazioneConferimenti = SecBidone.GetKeyValue("TipoVisualizzazioneConferimenti");
                    BidoneTmp.Count = SecBidone.GetKeyValue("Count");
                    BidoneTmp.OrdineVisualizzazione = SecBidone.GetKeyValue("OrdineVisualizzazione");
                } else
                    break;
            }
            return true;
    ...
    public class IniFile {
        ...
        private OrderedDictionary m_sections; // Hashtable

        // Public constructor
        public IniFile() {
            m_sections = new OrderedDictionary(StringComparer.InvariantCultureIgnoreCase);
        }

        // Loads the Reads the data in the ini file into the IniFile object
        public void Load(Stream sFileName, bool bMerge = false) {
            if (!bMerge)
                RemoveAllSections();
            // Clear the object... 
            ...
            while (!oReader.EndOfStream) {
                ...
                if (line != string.Empty) {
                    Match m = null/* TODO Change to default(_) if this is not a reference type */;
        ...
        public void Save(Stream sFileName) {
            StreamWriter oWriter = new StreamWriter(sFileName);
        ...
        public IniSection AddSection(string sSection) {
            ...
            // Trim spaces
            if (m_sections.Contains(sSection))
        ...
        public class IniSection {
            ...
            protected internal IniSection(IniFile parent, string sSection) {
                m_pIniFile = parent;
                m_sSection = sSection;
                m_keys = new OrderedDictionary(StringComparer.InvariantCultureIgnoreCase);
            }
            ...
            public void SetKeyValue(string sKey, string Value) {
                m_pIniFile.SetKeyValue(m_sSection, sKey, Value);
            }
